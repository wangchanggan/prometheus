...
func (sp *scrapePool) sync(targets []*Target) {
	...
	for _, t := range targets {
		hash := t.hash()
		// 判断在scrapePool中是否存在Target
		// 如果不存在,就创建targetscraper和ScrapeLoop,然后将ScrapeLoop加入scrapePool中，并启动ScrapeLoop
	...
	// 删除已下线的Target和scrapeLoop，并停止ScrapeLoop服务
